...
const ConversorDolarParaReal = () => {
  ...
  const taxaDeCambioAtual = JSON.stringify(jsonData, null, 2);
  const valorReal = jsonData.usd.brl;

  const converterParaReal = () => {
    setValorConvertido(`R$ ${valorReal}`);
  };

  return (
    <ScrollView style={styles.container}>
      <Button title="USD -> BRL" onPress={converterParaReal} style={styles.button} />
      <Text style={styles.resultado}>{valorConvertido}</Text>
      <Text style={styles.jsonData}>{taxaDeCambioAtual}</Text>
    </ScrollView>
  );
};
